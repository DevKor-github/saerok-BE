...
CREATE TABLE user_bird_collection_like (
    ...
    CONSTRAINT uq_user_bird_collection_like_user_collection UNIQUE (user_id, collection_id)
);
...
ALTER TABLE user_bird_collection_like
    ...
    ADD CONSTRAINT fk_user_bird_collection_like_collection FOREIGN KEY (collection_id) REFERENCES user_bird_collection(id) ON DELETE CASCADE;

-- 인덱스 생성
-- 좋아요 누른 컬렉션 목록 조회, 좋아요 누른 유저 조회 시에 이용
CREATE INDEX idx_user_bird_collection_like_user ON user_bird_collection_like(user_id);
CREATE INDEX idx_user_bird_collection_like_collection ON user_bird_collection_like(collection_id);
